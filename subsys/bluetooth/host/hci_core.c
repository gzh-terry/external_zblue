...
int bt_hci_cmd_send(uint16_t opcode, struct net_buf *buf)
{
	...
	BT_INFO("%s: %d, pid: %d", __func__, __LINE__, getpid());
	BT_INFO("cmd send opcode 0x%04x len %u", opcode, buf->len);
			 ...
			 struct net_buf **rsp)
{
	...
	BT_INFO("send sync buf %p opcode 0x%04x len %u", buf, opcode, buf->len);
	...
	if (status) {
		BT_ERR("opcode 0x%04x status 0x%02x", opcode, status);
		net_buf_unref(buf);
	...
	BT_INFO("rsp %p opcode 0x%04x len %u", buf, opcode, buf->len);
...
static void hci_disconn_complete_prio(struct net_buf *buf)
{
	...
	BT_INFO("status 0x%02x handle %u reason 0x%02x", evt->status, handle,
	       evt->reason);
...
static void hci_cmd_complete(struct net_buf *buf)
{
	...
	BT_INFO("cmd comp opcode 0x%04x", opcode);
...
static void hci_cmd_status(struct net_buf *buf)
{
	...
	BT_INFO("cmd stat opcode 0x%04x", opcode);
...
static void send_cmd(void)
{
	...
	/* Wait until ncmd > 0 */
	BT_INFO("calling sem_take_wait :%d", k_sem_count_get(&bt_dev.ncmd_sem));
	...
	BT_INFO("Sending command 0x%04x (buf %p) to driver", cmd(buf)->opcode, buf);
